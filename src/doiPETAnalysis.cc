...
doiPETAnalysis::doiPETAnalysis()
{
	...
	//Set energy window
	lowerThreshold = 435*keV;
	upperThreshold = 650*keV;
	...
	//
	coincidence_window = 4.7 * ns; //default value is zero, which means no coincidence window is applied
	...
	crystalResolutionMin = 0.10;//13%
	crystalResolutionMax = 0.10;//17%
...
void doiPETAnalysis::ReadOut(G4int blkID, G4int cryID, G4double interTime, G4double timeAnnih, G4ThreeVector interPos, G4double edep)
{
	...
	if(totalEdep>lowerThreshold && totalEdep<upperThreshold ){
		...
		if(getCoincidenceData){
			if (eventID_coin.empty() || abs(timeStamp - time_coin.back()) < coincidence_window) {
			...
			else if (!eventID_coin.empty() && abs(timeStamp - time_coin.back()) >= coincidence_window) {
				eventID_coin.clear();
				blockID_coin.clear();

...
void doiPETDetectorConstruction::DefineMaterials()
{
	...
	Lu = nist->FindOrBuildElement("Lu", isotopes);
	...
	LSO->AddElement(Si, 1);
	LSO->AddElement(O,  5);
	crystalMaterial = nist->FindOrBuildMaterial("LSO");
}
...
void doiPETDetectorConstruction::ConstructPhantom(G4LogicalVolume* worldLogical)
{
	...
	else if (PhantomType == "dicom") 
	{
		#ifdef G4_DCMTK
			ReadPhantomDataNew();
			ConstructPhantomContainerNew(worldLogical);
		#else
			ReadPhantomData();
...
void doiPETDetectorConstruction::ConstructPhantomContainerNew(G4LogicalVolume* worldLogical)
{
	#ifdef G4_DCMTK
...
void doiPETDetectorConstruction::ConstructPhantomContainer()
{
	//---- Extract number of voxels and voxel dimensions
...
void doiPETDetectorConstruction::ReadPhantomData()
{
...
void doiPETDetectorConstruction::ReadPhantomDataNew()
{
	#ifdef G4_DCMTK
	...
	G4cout << "GmReadPhantomG4Geometry::ReadPhantomData fNoVoxels X/Y/Z " << fNoVoxelsX << " " << fNoVoxelsY << " " << fNoVoxelsZ << G4endl;
	fin >> fMinX >> fMaxX;
	fin >> fMinY >> fMaxY;
	fin >> fMinZ >> fMaxZ;
	...
	#  ifdef G4VERBOSE
	G4cout << " Extension in X " << fMinX << " " << fMaxX << G4endl << " Extension in Y " << fMinY << " " << fMaxY << G4endl << " Extension in Z " << fMinZ << " " << fMaxZ << G4endl;
	#  endif
...
void doiPETDetectorConstruction::ReadVoxelDensities(std::ifstream& fin)
{
	G4String stemp;
...
void doiPETDetectorConstruction::ReadPhantomDataFile(const G4String& fname)
{
	G4cout << " DicomDetectorConstruction::ReadPhantomDataFile opening file " << fname << G4endl;  // GDEB
...
void doiPETDetectorConstruction::MergeZSliceHeaders()
{
...
G4Material* doiPETDetectorConstruction::BuildMaterialWithChangingDensity(const G4Material* origMate, G4float density, G4String newMateName)
{
